...
class CameraSettingsWindow(qt.QMainWindow):
    """Window for setting the camera."""
    buttonClicked = qt.Signal()

    def __init__(self, camera_init: CameraInit):
        super().__init__()

        if camera_init is not None:
            self.camera_init = camera_init
            if self.camera_init.getBackend() == "DSHOW":
                self.camera_init.openDSHOWSettings()
